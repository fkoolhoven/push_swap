...
/*   Updated: 2023/03/29 12:18:20 by fkoolhov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_stack	*find_longest_list(t_stack **stack_a, char asc_or_desc)
{
	...
	while (current_node)
	{
		...
		{
			found_pile = find_pile(current_node, pile_top, &first_top, asc_or_desc);
			if (!found_pile)
	...
	list_start = tag_nodes_as_part_of_list(&pile_top, asc_or_desc);
	return (list_start);
}
